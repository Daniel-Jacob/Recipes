...
<br>
- processFinish: after asynctask is done the recipes are sent to this function. This function in turn sends it to the returnRecipesToGridview function within the utils class. The Reason for this is because this function is implemented twice. 
...
<br>
<b>Challenges</b>
in the design document I made the choice for Spoonacular API because this supported the most functionality, and was best documented. However they were experiencing technical difficulties, so I had to change my choice of API. I picked yummly because it was reasonably documented, albeit with less functionality (autocomplete is not supported for example). Therefore I have not implemented autocomplete functionality. 
RecipeActivity and RecipeByIngredientActivity: There is no generalRecipes function, because 

differentiation between general recipes and recipes by ingredient gone. The reason is because the async task is implemented with an interface, so there is only 1 async task for two activities. The only difference is the endpoint and the recipes that are returned. Therefore, it is also not necessary to have a recipesByIngredient object, because both endpoints return recipes, they are just different in which recipes are returned. There is however still a populateGridview function, which is called toGridView and is implemented in the utils class. 

DetailsActivity: 
In the design document I had more fields that I wanted to use to display details. However I have not implemented preparation, type of food and calories. This is partially because of time constraint. 

FavoritesActivity:
- have not implemented the seperation between eaten and not eaten recipes

The main part that I have noticed during this process is that when starting to code up the application everything seems to go smoothly, and it seems like I am almost done. Then because of the decisions I have made, I need to practically rebuild the entire backend, because of an important detail I did not think about at the beginning of development. This detail was not allowing a duplicate entry to enter the favorites list. Since I added only titles in the database I could not check if a given object was equal to another object. (it is possible that two objects have the same title but this doesn't mean it is the same recipe). I therefore needed to store the entire object in firebase instead of just the titles. After that was done, I would also need to create a custom adapter instead of the arrayAdapter I was using because at this point I now get objects back instead of an Arraylist of strings, so this also needed to be redone. Something else I have learned is that in the process of getting a better grade on better code hub I have wrapped lots of code in functions, but during this process the code has become considerabely more complicated. Next time I would like to find a better balance between keeping the code simpel and writing short units of code. I think it is more important that the code is understandable, than writing units of 15 lines. 
